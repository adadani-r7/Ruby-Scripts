...
sites_to_import.each do |site_import|
  ...
  begin
    ...
    engine = engine_listing.select{|engine_summary| engine_summary.name == site_import.engine}
    enginepool = engine_pools.select{|engine_summary| engine_summary.name == site_import.engine}

    log.debug "engine_pool_info=#{engine}"
    if !engine[0].nil?
      site.engine_id = engine[0].id
      log.debug "engine set #{engine[0].id}"
    elsif !enginepool[0].nil?
      site.engine_id = enginepool[0].id
      log.debug "engine pool set #{enginepool[0].id}"
    end
