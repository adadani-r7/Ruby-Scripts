...
CSV.foreach('JuneScanSchedule.csv', {:headers => true, :encoding => "ISO-8859-15:UTF-8"}) do |row|
  ##puts row
  name = row['Site Name']
...
sites_to_import.each do |site_import|
  ...
  begin
    ...
    if site_import.schedule.to_s != ""
      ...
      time_pst = time + 25200
	  #time_est = time + 14400
      time_formatted = time_pst.strftime("%Y%m%dT%H%M00000")
      ...
      site.schedules = [] #overwrites the existing schedule
